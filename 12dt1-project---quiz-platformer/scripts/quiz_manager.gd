...
# Starts the quiz when it loads, shows a random question 
# Tells you if the previous answer was correct or not
func _ready():
	...
	if Gamescore.last_answer_message != "":
		...
		Gamescore.last_answer_message = ""  
		
	#Loads the character based on character selection
	match SelectionCharacter.selected_character:
		"Character1":
			$"../Player/SingleChicken".texture = preload("res://assets/single_chicken.png")
		"Character2":
			$"../Player/SingleFish".texture = preload("res://assets/single_fish.png")
		"Character3":
			$"../Player/SinglePenguin".texture = preload("res://assets/single_penguin.png")
		"Character4":
			$"../Player/SingleShark".texture = preload("res://assets/single_shark.png")


func _on_Timer_timeout():
	answer_checker.visible = false
	
	# Selects a random question, to show to the end user
...
# Labels for the questions and answer display
func show_question(index):
	...
	$C.text = q["options"][2]
	$D.text = q["options"][3]
# Checks for whether or not the answer is correct
# Increase the score if correct, reset if incorrect
...
#Detects what the player answered, by having them enter a collision shape
